...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  """
  desired_samples = int(sample_rate * clip_duration_ms / 1000)			# clip 1000
  window_size_samples = int(sample_rate * window_size_ms / 1000)		# window 30
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  fingerprint_4d = tf.reshape(fingerprint_input,
                              [-1, input_time_size, input_frequency_size, 1])       # 3000+  ->  -1,time_size=spectrogram_length:,frequency_size=dct_coefficient_count:40,1
							      # spectrogram_length = 1 + int(length_minus_window / window_stride_samples)
  first_filter_width = 8
  first_filter_height = 20
  first_filter_count = 64
  first_weights = tf.Variable(
      tf.truncated_normal(  # why  truncated_normal?   to the dimension
