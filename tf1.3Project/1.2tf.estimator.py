...
train_metrics = estimator.evaluate(input_fn=train_input_fn) # using the original training set to evaluate, so it is good, loss = 3.54e-08
eval_metrics = estimator.evaluate(input_fn=eval_input_fn) # using the different set to evaluate, so it is not so good, loss=0.002
