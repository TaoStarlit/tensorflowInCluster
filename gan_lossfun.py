...
for it in range(1000000):
    ...
    label=Variable(torch.Tensor(mb_size,1))
    ...
    D_loss1 = BCE(torch.sigmoid(D_real), label)
    D_loss1.backward()
    label.data.fill_(0.0)
    D_loss2 = BCE(torch.sigmoid(D_fake), label)
    D_loss2.backward()
    ...
    """ Common One """
    label.data.fill_(1.0)
    G_loss = BCE(torch.sigmoid(D_fake), label)
    """ Total Variation """
